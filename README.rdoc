...
To start the Hare process, execute `./bin/hare start` to start the hare process as a daemon, or `./bin/hare run` to keep the output attached to your terminal. I also recommend trying the foreman gem, so you can conveniently run several processes (like Hare and the Rails server) at the same time.
